...
public class Controller_XR : MonoBehaviour
{
    ...
    public void PlaceInXrRigSpawnedObjects(GameObject go)
    {
        if (go)
        {
//            Dev.Log("[Controller_XR]" + go.name + " GO container exists in XrRig");
        }
        else
        {
        ...
        if (_spawnedObjects)
        {
//            Dev.Log("[Controller_XR]" + _spawnedObjects.name + " GO exists");
        }
        else
        {
    ...
    public int GetLastPositionCount()
    {
        return _lastPositions.Count;
    }
