using System.Collections;
using System.Collections.Generic;
using UnityEngine;

/// <summary>
/// 
/// </summary>
public class VES_HeadEquipmentLocation : MonoBehaviour
{
    public ENUM_XROS_VES_Layer myVESLayer;
    
    public GameObject Socket_Mouth;
    public GameObject Socket_Eye_Both;
    public GameObject Socket_Eye_Left;
    public GameObject Socket_Eye_Right;
    public GameObject Socket_Ear_Left;
    public GameObject Socket_Ear_Right;
    public GameObject Socket_Mask;
    public GameObject Socket_Avatar;
    // Start is called before the first frame update
    void Start()
    {
        
    }

    // Update is called once per frame
    void Update()
    {
        
    }
}
